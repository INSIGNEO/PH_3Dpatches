...
####################
	...
	def create_dataset(self,split_max=0):
		"""
		...
		if series=={}:
			...
			if self.type_analysis=='SE':
				X =images_total
				Y=(mask_total)
				Xtotal=images_total_full
			else:
				X = np.reshape(np.array(images_total), [images_total.shape[0], self.image_shape, self.image_shape, self.channels])
			# store the masks
			...
			if self.data_extention!="nii.gz":
				Xtotal = np.reshape(np.array(images_total_full), [(images_total_full.shape[0]), self.original_image_shape, self.original_image_shape, self.channels])
			
		print('Dataset shape :', X.shape, Y.shape)
		#print(np.array(X[1,:10,:10,0]))
	...
	def load_images(self,contours_list_series,case=None,serie=None):
		...
		images_fullsize=[]
		c=[]
		print("list image: ",(contours_list_series))
		for o in range(0,len(contours_list_series)):	
			...
			if self.data_extention=='jpeg' or self.data_extention=='png' or self.data_extention=='tif' or self.data_extention=='jpg':
				...
				if self.data_extention=='nii' or self.data_extention=='nii.gz':
					...
					#affine= np.eye(4)
					#img=img1.get_fdata()
					#imgo = nib.Nifti1Image(img.astype(np.float64), affine)
					im=img1.get_fdata()
					if im.shape[2]==1 or im.shape[0]==1:
						#print(im.shape)
						im=np.squeeze(im)
						a=(im)
					else:
			...
			if o!=0:
				if len(images.shape)==3:
					...
					image_p=np.reshape(image_p,[image_p.shape[0],image_p.shape[1],image_p.shape[2],1])
					images_fullsize=np.reshape(images_fullsize,[images_fullsize.shape[0],images_fullsize.shape[1],images_fullsize.shape[2],1])
					full_image=np.reshape(full_image,[full_image.shape[0],full_image.shape[1],full_image.shape[2],1])
					#print(images.shape)
				if self.data_extention!="nii.gz":
			...
			if (self.type_analysis=='SE'):					
				if self.counter_extention=='txt':
					...
					if self.counter_extention=='nii' or self.counter_extention=='nii.gz' :
						img1 = nib.load(each_contour_path)
						#affine=np.eye(4)
						#img1=img1.get_fdata()
						#imgp = nib.Nifti1Image(img1.astype(np.float64), affine)
						im=img1.get_fdata()
						length=1
						if im.shape[2]==1 or im.shape[0]==1:
							#print(im.shape)
				...
				if self.counter_extention=='nii.gz':
					Ymask= np.reshape(np.array(Ymask1,dtype='uint8'), [ self.roi_shape, self.roi_shape, Ymask1.shape[2]])
					contours_mask=np.reshape(np.array(contours_masks,dtype='uint8'), [self.original_image_shape, self.original_image_shape,Ymask1.shape[2]])
...
####################
	...
	def quick_use_data(self,series,split_max=0):
		"""
		...
		for case, serie in series.items():
			...
			for ser in serie: 
				...
				if split_max!=0:
					...
					contours_list_to=contours_list_total[start:]
					contours_list=contours_list_to[:end]
				else:
				...
				if self.type_analysis=='CL':
					...
					if split_max!=0:
						contours_list212=contours_list21[start:]
						contours_list2=contours_list212[:end]
					else:
				...
				mask,cmask= self.load_masks((contours_list2),folder=folder)
				if(o==0):
					cmask_total=cmask
					mask_total=mask
					images_total=images
		...
		if self.type_analysis=='SE':
			if self.counter_extention=='nii.gz':
				X_total=np.reshape(X_total, [X_total.shape[0], X_total.shape[1], X_total.shape[2]])
				cX_total=np.reshape(cX_total, [cX_total.shape[0], cX_total.shape[1], cX_total.shape[2]])
				Y_total=np.reshape(Y_total, [Y_total.shape[0], Y_total.shape[1], Y_total.shape[2]])
				cY_total=np.reshape(cY_total, [cY_total.shape[0], cY_total.shape[1], cY_total.shape[2]])
			else:
...
class create_dataset_itter:
	...
	def __init__ (self,split=0,analysis='train',path_case='main') :
		self.step=split
	...
	def __next__(self):
		if self.i<self.max:
			...
			X_total,cX_total,Y_total,cY_total=[],[],[],[]
			X_total,cX_total,Y_total,cY_total,TXT=dsn.create_dataset(split_max=self.step)
			#print(np.array(X_total[1,:10,:10,0]))
