...
##################################################################################################################################
		...
		if self.type_analysis=='CL':
			...
			for o in range(len(model_name)): 
				...
				with open(self.store_model_path+ns, mode='w') as employee_file:
					employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
					y_pred_str=zip([map(str,y_t[:,u]) for u in range(len(np.array(y_t).shape))])
				...
				with open(self.store_model_path+ns2, mode='w') as employee_file2:
					...
					y_r_str=zip([map(str,mainY2[:,u]) for u in range(len(np.array(mainY2).shape))])
					for row2 in y_r_str:
	...
	def train_vit(self,model_vit='res'):
		...
		tconf = TrainerConfig(max_epochs=40, batch_size=64, learning_rate=1e-3)
		# sample model config.
		model_config = {"image_size":32, "patch_size":4, "num_classes":classes, "dim":64, "depth":3, "heads":4, "mlp_dim":128, 'channels':3, 'model_case':model_vit}
...
#	def test_vit(self,model_vit=='res'):
